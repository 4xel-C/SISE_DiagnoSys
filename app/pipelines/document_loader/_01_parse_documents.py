...
def parse_documents(raw_docs, target_types=None):
    parsed = []
    for doc in raw_docs:
        relevant_sections = doc.get("content", {}).get("sections", [])
        link = doc.get("link")
        for section in relevant_sections:
            logger.info(f"Section '{section.get('section_title', '')}' trouvée dans doc '{doc.get('title', '')}'")
        parsed.append({
            "title": doc.get("title"),
            "date": doc.get("date"),
        ...
        logger.info(f"Parsed document: {doc.get('title', '')} with {len(relevant_sections)} sections.")
